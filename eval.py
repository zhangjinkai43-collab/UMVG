...
def get_args_parser():
    ...
    parser.add_argument('--profile_only', action='store_true',
                        help='only compute Params and GFLOPs with dummy inputs, then exit')
...
def main(args):
    ...
    if args.profile_only:
        try:
            ...
            dummy_img = torch.randn(1, 3, args.imsize, args.imsize, device=device)
            dummy_mask = torch.zeros(1, args.imsize, args.imsize, dtype=torch.bool, device=device)
            img_data = NestedTensor(dummy_img, dummy_mask)

            dummy_text = torch.randint(0, 30522, (1, args.max_query_len), device=device)
            dummy_text_mask = torch.zeros(1, args.max_query_len, dtype=torch.bool, device=device)
            text_data = NestedTensor(dummy_text, dummy_text_mask)
