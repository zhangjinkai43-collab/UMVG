...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='nautdata', type=str,
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 帮 Json 解包，把 tensor 强转成普通 float
        log_stats = {**{f'train_{k}': v.item() if isinstance(v, torch.Tensor) else v for k, v in train_stats.items()},
                     **{f'validation_{k}': v.item() if isinstance(v, torch.Tensor) else v for k, v in
                        val_stats.items()},
                     'epoch': epoch,
